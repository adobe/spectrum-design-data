...
2. **Name-object scope fields** on a token (`component`, `variant`, `anatomy`, `interaction`, `interaction-context`, and arbitrary option keys; token-declares-scope direction, validated today by SPEC-018, SPEC-019, SPEC-020, SPEC-022, and SPEC-040).
...
[
  {
    "scope": {
      ...
      "part": "label",
      "property": "color",
      "options": { "variant": "accent", "interaction": ["hover"] }
    },
...
Layer 1 (`relationship.schema.json`) only enforces shape: `scope.options` is an object, `scope.options.interaction`/`scope.options.interaction-context` (when present) are arrays of strings, and other option values are typically strings. It does **not** enforce which option values are valid — that is a Layer 2 (Rust, `sdk/core`) relational rule, because it requires cross-referencing the component catalog.
...
| SPEC-054 | `ctr-state-valid`                 | error    | CTR `scope.options.interaction`/`scope.options.interaction-context` array elements **MUST** match the `name` of a declared state on the referenced component (when state declarations are present).                                                |
