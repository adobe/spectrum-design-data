...
| See mode-set values (color scheme / scale / contrast) up front       | `primer` → `modeSets`                                                              | agent           | ❌             | Always `{colorScheme: [], scale: [], contrast: []}` on the embedded dataset even though `query_tokens filter="colorScheme=dark"` proves the field is populated. (bead `spectrum-design-data-v9bb`) |
...
| Ask in plain language ("background for a selected, hovered row")     | `design-data-suggest intent="…"`                                                   | **public only** | ⚠️            | Works on the public server (Jaccard ranking). **Not available** on the wired agent server — no first-class equivalent. (bead `spectrum-design-data-st8c`)                                          |
