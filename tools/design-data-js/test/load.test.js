...
test.before(() => {
  ...
  writeFileSync(join(TMP, 'base.tokens.json'), JSON.stringify([SAMPLE_TOKEN_1]), 'utf-8');
  writeFileSync(join(TMP, 'sub', 'extra.tokens.json'), JSON.stringify([SAMPLE_TOKEN_2]), 'utf-8');
  // A non-cascade file that should be skipped.
  writeFileSync(join(TMP, 'legacy.json'), JSON.stringify({ key: { value: '#abc' } }), 'utf-8');
});
...
test('loadDataset tokens are queryable', async (t) => {
  ...
  t.is(results[0].name, 'background-color');
});
