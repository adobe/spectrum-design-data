...
pub struct RegistryData {
    ...
    /// Anatomy-term ids flagged `standaloneScope: true` in `anatomy-terms.json` —
    /// terms permitted to scope a token's name object on their own (SPEC-025), with
    /// no accompanying `component` or `structure` field. Cross-cutting parts like
    /// `focus-ring`/`focus-indicator` have no single owning component.
    anatomy_standalone_scope: HashSet<String>,
}
...
/// Parse a registry JSON file's ids flagged `standaloneScope: true`.
///
/// Used for `anatomy-terms.json`: a handful of cross-cutting anatomy parts
/// (`focus-ring`, `focus-indicator`) have no single owning component or structure,
/// so SPEC-025 allows them to scope a token's name object on their own.
fn parse_standalone_scope_ids(json_str: &str) -> HashSet<String> {
    let v: Value = serde_json::from_str(json_str).expect("embedded registry JSON must be valid");
    let mut set = HashSet::new();
    if let Some(values) = v.get("values").and_then(|v| v.as_array()) {
        for entry in values {
            if entry.get("standaloneScope").and_then(|v| v.as_bool()) == Some(true) {
                if let Some(id) = entry.get("id").and_then(|v| v.as_str()) {
                    set.insert(id.to_string());
                }
            }
...
impl RegistryData {
    ...
    pub fn embedded() -> &'static Self {
        ...
        INSTANCE.get_or_init(|| Self {
            ...
            anatomy_standalone_scope: parse_standalone_scope_ids(ANATOMY_TERMS_JSON),
        })
    }

    /// Anatomy-term ids flagged `standaloneScope: true` — see the field doc comment
    /// on [`RegistryData::anatomy_standalone_scope`].
    pub fn anatomy_standalone_scope_terms(&self) -> &HashSet<String> {
        &self.anatomy_standalone_scope
    }
